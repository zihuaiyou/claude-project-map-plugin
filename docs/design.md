...
- `providers/types.ts` 定义 `LanguageProvider` 接口（detect、extractExports、extractImports、recognizeFile、dirPurposeHints）
- 每种语言一个独立 module（`providers/java.ts`、`python.ts`、`go.ts`、`rust.ts`），实现接口
- `providers/registry.ts` 统一注册，供 MCP 工具遍历调用
- `detect_stack` 返回 `languages: string[]` 列出所有检测到的语言
- `analyze_key_files` 按文件扩展名路由到对应 provider 解析导出和配置
- `extract_arch_patterns` 源码扫描覆盖 `.java/.py/.go/.rs`，按语言解析导入

**新增语言流程：** 创建 `providers/<lang>.ts` 实现接口 → `registry.ts` 注册 → 无需改工具代码。
