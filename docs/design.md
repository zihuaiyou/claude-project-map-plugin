...
| 术语 | 说明 |
| ------ | ----- |
| **CLAUDE.md** | Claude 自动读取的项目地图文件 |
| **MCP Server** | Claude 的可插拔工具服务器（Model Context Protocol） |
| **Skill** | Claude 端的编排脚本，控制多步工作流 |
| **Token** | AI 模型计费单位，直接关联 API 费用 |
| **Git Ref** | 增量更新中记录的 commit hash |
| **Monorepo** | 多包单仓库的组织方式 |
