...
| 包级 | `--package <name>` | 限定范围到 `packages/<name>/` 或 `apps/<name>/` |

`--package <name>` 可与其他模式组合：`/update-map --package foo`、`/update-map --package foo --quick`、`/update-map --package foo --full`。
...
```
1. 读取当前 targetFile（如果有）→ 了解当前状态
2. 获取当前 git HEAD: git rev-parse HEAD → headHash
3. 调用 MCP tool: scan_structure（rootPath=pkgRoot）→ 获取目录树
4. 调用 MCP tool: analyze_key_files（rootPath=pkgRoot）→ 获取文件用途
5. 调用 MCP tool: detect_stack（rootPath=pkgRoot）→ 获取技术栈
6. 调用 MCP tool: extract_arch_patterns（rootPath=pkgRoot）→ 获取架构规则
7. 汇总 → 压缩 → 写入 targetFile（含 _git_ref: headHash）
8. 输出变更摘要
```
...
```
1. 读取当前 targetFile → 提取 _git_ref 值（记为 gitRef）
2. 获取当前 git HEAD: git rev-parse HEAD → headHash
3. 如果 gitRef === headHash → 输出「自上次更新无新提交，跳过」
4. 执行 git diff {gitRef}..HEAD --name-only --diff-filter=ACMR [-- pkgPath]
   → 新增/修改/重命名的文件列表（changedFiles）
5. 执行 git diff {gitRef}..HEAD --name-only --diff-filter=D [-- pkgPath]
...
6. 如果 changedFiles 和 deletedFiles 均为空：
   ...
   → 目录树有变化 → 只更新目录树部分
7. 有文件变更：
   7a. 调用 MCP tool: scan_structure（rootPath=pkgRoot）
   7b. 调用 MCP tool: analyze_key_files（rootPath=pkgRoot, filePaths=changedFiles）
   7c. 如果 package.json 在 changedFiles 中 → 调用 detect_stack（rootPath=pkgRoot）
   7d. 如果目录结构有显著变化 → 调用 extract_arch_patterns（rootPath=pkgRoot）
...
```
1. 读取当前 targetFile
2. 调用 MCP tool: scan_structure（rootPath=pkgRoot）→ 获取目录树
3. 与 targetFile 中目录树对比
...
当用户传入 `--package <name>`，自动计算目标路径：

```
--package foo      → pkgRoot = <projectRoot>/packages/foo/
                     targetFile = packages/foo/CLAUDE.md
                     pkgPath = packages/foo/    （git diff 路径前缀）
--package apps/web → pkgRoot = <projectRoot>/apps/web/
                     targetFile = apps/web/CLAUDE.md
                     pkgPath = apps/web/
无 --package       → pkgRoot = <projectRoot>
                     targetFile = CLAUDE.md
                     pkgPath = （空，不限路径）
```

所有 MCP 调用的 `rootPath` 统一替换为 `pkgRoot`。git diff 追加 `-- pkgPath` 限制范围。
...
使用 Read 工具读取 `targetFile`（有 `--package` 时读包级 CLAUDE.md，否则读根目录）。
...
```bash
# 新增/修改/重命名（有 --package 时追加路径范围）
git diff {gitRef}..HEAD --name-only --diff-filter=ACMR {pkgPath:+-- $pkgPath}

# 已删除
git diff {gitRef}..HEAD --name-only --diff-filter=D {pkgPath:+-- $pkgPath}
```
...
所有 MCP 工具的 `rootPath` 统一为 `pkgRoot`。
...
使用 Write 工具将压缩后的内容写入 `targetFile`。

- 有 `--package`：写入 `packages/<name>/CLAUDE.md` 或 `apps/<name>/CLAUDE.md`
- 无 `--package`：写入根目录 `CLAUDE.md`
...
如果是跳过模式：
```markdown
⏭️ {targetFile} 无需更新 — 自 _git_ref {shortHash} 起无变化
```
...
- ☑ `--package <name>` 对应路径不存在时，报错并列出可用包
